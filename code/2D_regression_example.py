...
for i in range(N_grid):
    for j in range(N_grid):
        ...
        X[n,:] = x
        y = np.sin(x[0]) + np.cos(x[1]) + 0.2*np.random.randn() # True function is f(x)=sin(x_1)+cos(x_2)
        Y_grid[i,j] = y 
        Y[n] = y
        n += 1
...
N_Star = 30     # Input points for predictions will be distributed across a 30x30 grid 
r_star = np.linspace(0,10,N_Star)   # Input points for predictions range from 0 to 10 in both directions
X_star1, X_star2 = np.meshgrid(r_star,r_star)   # Form mesh grid for X_star
Y_StarMean = np.zeros([N_Star,N_Star])          # Initialise mean of GP predictions
Y_StarStd = np.zeros([N_Star,N_Star])           # Initialise std of GP predictions
for i in range(N_Star):
    for j in range(N_Star):
        x_star = [X_star1[i,j],X_star2[i,j]]
        Y_StarMean[i,j], Y_StarStd[i,j] = GP.Predict(X,x_star,L,Sigma,Y,K,C,InvC,N)
